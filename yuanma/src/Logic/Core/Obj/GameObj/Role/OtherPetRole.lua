...
function OtherPetRole:ctor()
    ...
    self._kRoleType = kType.kRole.kOtherPet         -- 宠物角色类型
...
function OtherPetRole:create(pRoleInfo,charTag,master,cooperateInfo)
...
function OtherPetRole:dispose(pRoleInfo,charTag,master,cooperateInfo)     
    ------------------- 初始化 ----------------------
    -- 设置宠物角色信息
...
function OtherPetRole:initInfo(pRoleInfo, charTag, master,cooperateInfo)
...
-- 初始化特效
function OtherPetRole:initEffects()
...
-- 移除特效动画
function OtherPetRole:removeAllEffects()
    if cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then  
...
-- 初始化引用计数
function OtherPetRole:initRefs()
    if cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
        -- 创建无视攻击状态引用计数
...
-- 播放受击特效
function OtherPetRole:playHurtedEffect(type, intersection, isCritical, isBlock)
...
-- 显示出场特效
function OtherPetRole:playAppearEffect()
    -- 刷新zorder
...
-- 显示死亡特效
function OtherPetRole:playDeadEffect()
    -- 刷新zorder
...
-- 初始化人物血条
function OtherPetRole:initHpBar()
    if cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
...
function OtherPetRole:refreshZorder()
    ...
    elseif cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
        -- 当前不为死亡状态时都会刷新zorder
        if self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole)._pCurState._kTypeID ~= kType.kState.kBattleOtherPetRole.kDead then
            if self._bForceMinPositionZ == true then   -- 强制positionZ
                self:setPositionZ(self._nForceMinPositionZValue)
            else                                    -- 非强制positionZ
                self:setPositionZ(self:getPositionIndex().y*(self:getMapManager()._f3DZ))
...
-- 设置battleuilayer 代理
function OtherPetRole:setBattleUILayerDelegate( delegate )
...
-- 设置血量
function OtherPetRole:setHp(value,maxValue) 
...
-- 加血
function OtherPetRole:addHp(value)
    if value <= 0 then
        return
    end
    value = math.ceil(value)
...
-- 参数1：掉血值
-- 参数2：是否暴击
function OtherPetRole:loseHp(value, isCritical)
...
-- 显示数字
function OtherPetRole:showNum(flag, textNum, isCritical)
    local fntFileName = ""
...
-- 显示暴击特效
function OtherPetRole:showCriticalEffect(intersection)
    -- 刷新zorder
...
-- 显示格挡特效
function OtherPetRole:showBlockEffect(intersection)
    -- 刷新zorder
...
-- 显示闪避特效
function OtherPetRole:showMissEffect()    
    -- 刷新zorder
...
function OtherPetRole:initStateMachine()
    ...
    elseif cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
        pStateMachine = require("BattleOtherPetRoleStateMachine"):create(self)
    end
    self._pStateMachineDelegate:addStateMachine(pStateMachine)
end

-- 创建人物角色控制机
function OtherPetRole:initControllerMachine()
    if cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
...
-- 设置当前移动速度比例（有叠加效果）
-- 参数：0到1 表示  0%到100%
function OtherPetRole:setCurSpeedPercent(percent)
    self._nCurSpeed = self._nCurSpeed * percent
end

-- 复位角色速度
function OtherPetRole:resetSpeed()
    self._nCurSpeed = self._pRoleInfo.Speed
end

-- 设置当前可能受到的所有伤害的比例（有叠加效果）
-- 参数：0到1 表示  0%到100%
function OtherPetRole:setCurHurtedPercent(percent)
    self._fCurHurtedRate = self._fCurHurtedRate * percent
end

-- 复位角色伤害比例
function OtherPetRole:resetHurtedRate()
    self._fCurHurtedRate = 1.0
end

-- 播放出场动作
function OtherPetRole:playAppearAction()
    -- 出场动作
...
-- 获取出场动作的时间间隔（单位：秒）
function OtherPetRole:getAppearActionTime()
...
-- 播放受击动作
function OtherPetRole:playBeatenAction()
    -- 受击动作
...
-- 获取受击动作的时间间隔（单位：秒）
function OtherPetRole:getBeatenActionTime()
...
-- 播放倒地动作
function OtherPetRole:playFallGroundAction()
    -- 倒地动作
...
-- 获取倒地动作的时间间隔（单位：秒）
function OtherPetRole:getFallGroundActionTime()
...
-- 播放起身动作
function OtherPetRole:playUpGroundAction()
    -- 起身动作
...
-- 获取起身动作的时间间隔（单位：秒）
function OtherPetRole:getUpGroundActionTime()
...
-- 播放眩晕动作
function OtherPetRole:playDizzyAction()
    -- 眩晕动作
...
-- 获取眩晕动作的时间间隔（单位：秒）
function OtherPetRole:getDizzyActionTime()
...
-- 播放死亡动作
function OtherPetRole:playDeadAction()
    -- 死亡动作
...
-- 获取死亡动作的时间间隔（单位：秒）
function OtherPetRole:getDeadActionTime()
...
-- 播放攻击动作
function OtherPetRole:playAttackAction(index)
    -- 攻击动作
...
-- 获取攻击动作的时间间隔（单位：秒）
function OtherPetRole:getAttackActionTime(index)
...
function OtherPetRole:isUnusualState()
    local state = self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole)._pCurState
    if state._kTypeID ~= kType.kState.kBattleOtherPetRole.kDead and
       state._kTypeID ~= kType.kState.kBattleOtherPetRole.kBeaten and
       state._kTypeID ~= kType.kState.kBattleOtherPetRole.kAppear and 
       state._kTypeID ~= kType.kState.kBattleOtherPetRole.kFrozen and 
       state._kTypeID ~= kType.kState.kBattleOtherPetRole.kDizzy and
       self._nCurHp > 0 then
        return false
    end
    return true
...
-- 设置战斗中的属性值的偏移变化值
function OtherPetRole:addAttriValueOffset(type,value)
    self._fAttriValueOffsets[type] = self._fAttriValueOffsets[type] + value
end

-- 获取战斗中当前的防御等级
function OtherPetRole:getCurDefenseLevel()
    return self._pCurDefLevel + self._nDefenseLevelOffset
end

-- 设置战斗中的需要添加的防御等级值变化偏移值
function OtherPetRole:addDefenseLevelOffset(value)
...
-- 技能影响到的受击接口
function OtherPetRole:beHurtedBySkill(skill, intersection)
    ...
    if skill._pSkillInfo.AttackMode == kType.kAttackMode.kDamage then  -- 伤害
        ...
        if self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole)._pCurState._kTypeID == kType.kState.kBattleOtherPetRole.kDead or
        ...
        if self._nCurHp == 0  then
            self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole):setCurStateByTypeID(kType.kState.kBattleOtherPetRole.kDead, true, {true,skill})
        ...
        -- 如果当前不为冻结状态，则考虑应值的情况 
        if self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole)._pCurState._kTypeID ~= kType.kState.kBattleOtherPetRole.kFrozen then
            -- 未被攻击的累积时间需要清零
            self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole)._fNoHurtTimeCounter = 0
            if skill._pSkillInfo.PowerValue then
                ...
                if self._nCurComboInterupt <= 0 then
                    ...
                    if self:isUnusualState() == false then  -- 非异常状态时，可以切入应值
                        self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole):setCurStateByTypeID(kType.kState.kBattleOtherPetRole.kBeaten, false, {skill, 5})
                    end
                    -- 屏幕添加卡顿，并添加破甲buff
            ...
            if attackerSkill._pSkillInfo.AttackLevel then
                ...
                if nAttackAndDefDiff > 0 then  -- 差值>0，切入应值状态
                    if self:isUnusualState() == false then  -- 非异常状态时，可以切入应值
                        self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole):setCurStateByTypeID(kType.kState.kBattleOtherPetRole.kBeaten, false, {attackerSkill, nAttackAndDefDiff})
                    end
                    return
                end
            end
...
-- buff影响到的受击接口
function OtherPetRole:beHurtedByBuff(buff)
    if self:getTalksManager():isCurTalksFinished() == false then        -- 正在显示剧情对话，则无视伤害
        return
    end
    -- 战斗结果已经得出，则不再有任何影响
    ...
    if loseHpValue ~= 0 then
        ...
        if self._nCurHp == 0  then
            self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole):setCurStateByTypeID(kType.kState.kBattleOtherPetRole.kDead, true, {false})
            return
        end
    end
