...
-- 创建函数
function OtherPlayerRole:create(pRoleInfo,charTag)
...
-- 处理函数
function OtherPlayerRole:dispose(pRoleInfo,charTag)    
    ------------------- 初始化 ----------------------
    -- 设置角色信息
...
-- 初始化信息
function OtherPlayerRole:initInfo(pRoleInfo, charTag)
...
function OtherPlayerRole:initAni()
    print("Other RoleID = "..self._pRoleInfo.roleId)
...
-- 初始化特效
function OtherPlayerRole:initEffects()
...
-- 移除特效动画
function OtherPlayerRole:removeAllEffects()
    if cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then  
...
-- 初始化引用计数
function OtherPlayerRole:initRefs()
    if cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
        -- 创建普通攻击按钮的引用计数
        self._refGenAttackButton = require("RoleGenAttackButtonRef"):create(self)
        -- 创建技能攻击按钮的引用计数
        for i=1,table.getn(self:getSkillsManager()._tOtherPlayerRolesMountActvSkillsInfos) do
...
-- 播放受击特效
function OtherPlayerRole:playHurtedEffect(type, intersection, isCritical, isBlock)
...
-- 显示死亡特效
function OtherPlayerRole:playDeadEffect()
    -- 刷新zorder
...
function OtherPlayerRole:refreshZorder()
    ...
    elseif cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
        -- 当前不为死亡状态时都会刷新zorder
        if self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole)._pCurState._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kDead then
            if self._bForceMinPositionZ == true then   -- 强制positionZ
                self:setPositionZ(self._nForceMinPositionZValue)
            else                                    -- 非强制positionZ
                self:setPositionZ(self:getPositionIndex().y*(self:getMapManager()._f3DZ))
...
-- 设置battleuilayer 代理
function OtherPlayerRole:setBattleUILayerDelegate( delegate )
    self._pBattleUIDelegate = delegate
    if self._strCharTag == "main" then
        if self._pBattleUIDelegate then
            self._pBattleUIDelegate:setMaxHp(self._nHpMax)
            self._pBattleUIDelegate:setCurHp(self._nHpMax,true)
        end
    end
end
...
-- 设置血量
function OtherPlayerRole:setHp(value,maxValue) 
    ...
    if self._strCharTag == "main" then
        if self._pBattleUIDelegate then
            self._pBattleUIDelegate:setMaxHp(self._nHpMax)
            self._pBattleUIDelegate:setCurHp(self._nCurHp, true)           
        end
    end
...
-- 加血
function OtherPlayerRole:addHp(value)
    ...
    if self._strCharTag == "main" then
        if self._pBattleUIDelegate then
            self._pBattleUIDelegate:setCurHp(self._nCurHp)
        end
    end
...
-- 参数1：掉血值
-- 参数2：是否暴击
function OtherPlayerRole:loseHp(value, isCritical)
...
-- 增长怒气
function OtherPlayerRole:addAnger(value)
...
-- 清空怒气
function OtherPlayerRole:clearAnger()
    self._nCurAnger = 0
end

-- 显示数字
function OtherPlayerRole:showNum(flag, textNum, isCritical)
    local fntFileName = ""
...
-- 显示暴击特效
function OtherPlayerRole:showCriticalEffect(intersection)
    -- 刷新zorder
...
-- 显示格挡特效
function OtherPlayerRole:showBlockEffect(intersection)
    -- 刷新zorder
...
-- 显示闪避特效
function OtherPlayerRole:showMissEffect()    
    -- 刷新zorder
...
function OtherPlayerRole:initStateMachine()
    ...
    elseif cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
        pStateMachine = require("BattleOtherPlayerRoleStateMachine"):create(self)
    end
    self._pStateMachineDelegate:addStateMachine(pStateMachine)
end

-- 创建人物角色控制机
function OtherPlayerRole:initControllerMachine()
    if cc.Director:getInstance():getRunningScene()._kCurSessionKind == kSession.kBattle then
...
-- 设置当前移动速度比例（有叠加效果）
-- 参数：0到1 表示  0%到100%
function OtherPlayerRole:setCurSpeedPercent(percent)
    self._nCurSpeed = self._nCurSpeed * percent
end

-- 复位角色速度
function OtherPlayerRole:resetSpeed()
    self._nCurSpeed = self._pRoleInfo.roleAttrInfo.speed
end

-- 设置当前可能受到的所有伤害的比例（有叠加效果）
-- 参数：0到1 表示  0%到100%
function OtherPlayerRole:setCurHurtedPercent(percent)
    self._fCurHurtedRate = self._fCurHurtedRate * percent
end

-- 复位角色伤害比例
function OtherPlayerRole:resetHurtedRate()
    self._fCurHurtedRate = 1.0
end

-- 播放出场动作
function OtherPlayerRole:playAppearAction()
    -- 出场动作
...
-- 获取出场动作的时间间隔（单位：秒）
function OtherPlayerRole:getAppearActionTime()
...
-- 播放受击动作
function OtherPlayerRole:playBeatenAction()
    -- 受击动作
...
-- 获取受击动作的时间间隔（单位：秒）
function OtherPlayerRole:getBeatenActionTime()
...
-- 播放倒地动作
function OtherPlayerRole:playFallGroundAction()
    -- 倒地动作
...
-- 获取倒地动作的时间间隔（单位：秒）
function OtherPlayerRole:getFallGroundActionTime()
...
-- 播放起身动作
function OtherPlayerRole:playUpGroundAction()
    -- 起身动作
...
-- 获取起身动作的时间间隔（单位：秒）
function OtherPlayerRole:getUpGroundActionTime()
...
-- 播放眩晕动作
function OtherPlayerRole:playDizzyAction()
    -- 眩晕动作
...
-- 获取眩晕动作的时间间隔（单位：秒）
function OtherPlayerRole:getDizzyActionTime()
...
-- 播放死亡动作
function OtherPlayerRole:playDeadAction()
    -- 死亡动作
...
-- 获取死亡动作的时间间隔（单位：秒）
function OtherPlayerRole:getDeadActionTime()
...
-- 播放攻击动作
function OtherPlayerRole:playAttackAction(index)
    -- 攻击动作
...
-- 获取攻击动作的时间间隔（单位：秒）
function OtherPlayerRole:getAttackActionTime(index)
...
function OtherPlayerRole:isUnusualState()
    local state = self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole)._pCurState
    if state._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kDead and
       state._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kBeaten and
       state._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kAppear and 
       state._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kFrozen and 
       state._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kDizzy and 
       self._nCurHp > 0 then
        return false
    end
    return true
...
-- 设置战斗中的属性值的偏移变化值
function OtherPlayerRole:addAttriValueOffset(type,value)
    self._fAttriValueOffsets[type] = self._fAttriValueOffsets[type] + value
end

-- 获取战斗中当前的防御等级
function OtherPlayerRole:getCurDefenseLevel()
    return self._pCurDefLevel + self._nDefenseLevelOffset
end

-- 设置战斗中的需要添加的防御等级值变化偏移值
function OtherPlayerRole:addDefenseLevelOffset(value)
...
function OtherPlayerRole:addPassiveByTypeID(typeID)
...
-- 技能影响到的受击接口
function OtherPlayerRole:beHurtedBySkill(skill, intersection)
    ...
    if skill._pSkillInfo.AttackMode == kType.kAttackMode.kDamage then  -- 伤害
        ...
        if self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole)._pCurState._kTypeID == kType.kState.kBattleOtherPlayerRole.kDead or
        ...
        if self._nCurHp == 0  then
            self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole):setCurStateByTypeID(kType.kState.kBattleOtherPlayerRole.kDead, true, {true, skill})
        ...
        -- 如果当前不为冻结状态，则考虑应值的情况 
        if self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole)._pCurState._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kFrozen then
            -- 未被攻击的累积时间需要清零
            self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole)._fNoHurtTimeCounter = 0
            if skill._pSkillInfo.PowerValue then
                ...
                if self._nCurComboInterupt <= 0 then
                    ...
                    if self:isUnusualState() == false then  -- 非异常状态时，可以切入应值
                        self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole):setCurStateByTypeID(kType.kState.kBattleOtherPlayerRole.kBeaten, false, {skill, 5})
                    end
                    -- 屏幕添加卡顿，并添加破甲buff
            ...
            if attackerSkill._pSkillInfo.AttackLevel then
                ...
                if nAttackAndDefDiff > 0 then  -- 差值>0，切入应值状态
                    if self:isUnusualState() == false then  -- 非异常状态时，可以切入应值
                        self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole):setCurStateByTypeID(kType.kState.kBattleOtherPlayerRole.kBeaten, false, {attackerSkill, nAttackAndDefDiff})
                    end
                    return
                end
            end
...
-- buff影响到的受击接口
function OtherPlayerRole:beHurtedByBuff(buff)
    if self:getTalksManager():isCurTalksFinished() == false then        -- 正在显示剧情对话，则无视伤害
        return
    end
    -- 战斗结果已经得出，则不再有任何影响
    ...
    if loseHpValue ~= 0 then
        ...
        if self._nCurHp == 0  then
            self:getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole):setCurStateByTypeID(kType.kState.kBattleOtherPlayerRole.kDead, true, {false})
            return
        end
    end
