...
-- All Rights Reserved.
--==================================================
-- filename:  PetSkill2.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/6/8
-- descrip:   宠物技能攻击2   红孩儿
--===================================================
local PetSkill2 = class("PetSkill2",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function PetSkill2:ctor()
    self._strName = "PetSkill2"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kPetSkill2            -- 技能对象类型
...
-- 创建函数
function PetSkill2:create(master,skillInfo)
    local skill = PetSkill2.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function PetSkill2:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function PetSkill2:onExitPetSkill2()    
    self:onExitSkillObj()
end

-- 循环更新
function PetSkill2:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function PetSkill2:initStateMachine()
...
-- 初始化动作帧事件回调
function PetSkill2:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function PetSkill2:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function PetSkill2:onEnterIdleDo(state)
...
-- 技能待机状态onExit时技能操作
function PetSkill2:onExitIdleDo()

end

-- 技能待机状态onUpdate时技能操作
function PetSkill2:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function PetSkill2:onEnterChantDo(state)
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function PetSkill2:onExitChantDo()

end

-- 技能吟唱状态onUpdate时技能操作
function PetSkill2:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function PetSkill2:onEnterProcessDo(state)
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function PetSkill2:onExitProcessDo()

end

-- 技能执行状态onUpdate时技能操作
function PetSkill2:onUpdateProcessDo(dt)

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function PetSkill2:onEnterReleaseDo(state)
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function PetSkill2:onExitReleaseDo()

end

-- 技能释放状态onUpdate时技能操作
function PetSkill2:onUpdateReleaseDo(dt)
