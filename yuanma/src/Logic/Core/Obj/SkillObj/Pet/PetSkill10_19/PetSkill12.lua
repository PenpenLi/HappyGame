...
-- All Rights Reserved.
--==================================================
-- filename:  PetSkill12.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/24
-- descrip:   宠物技能攻击12    六耳
--===================================================
local PetSkill12 = class("PetSkill12",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function PetSkill12:ctor()
    self._strName = "PetSkill12"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kPetSkill12            -- 技能对象类型
    self._pCurState = nil                                  -- 技能当前的状态机状态
    self._nRoleAttackActionIndex = 3                       -- 角色攻击动作index
    self._fChantDelayTime = 0.71                           -- 吟唱动作持续时间s
    self._fDistance = 150                                  -- 技能距离人物的距离
    
end

-- 创建函数
function PetSkill12:create(master,skillInfo)
    local skill = PetSkill12.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function PetSkill12:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function PetSkill12:onExitPetSkill12()    
    self:onExitSkillObj()
end

-- 循环更新
function PetSkill12:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function PetSkill12:initStateMachine()
...
-- 初始化动作帧事件回调
function PetSkill12:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function PetSkill12:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function PetSkill12:onEnterIdleDo(state)
...
-- 技能待机状态onExit时技能操作
function PetSkill12:onExitIdleDo()

end

-- 技能待机状态onUpdate时技能操作
function PetSkill12:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function PetSkill12:onEnterChantDo(state)
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function PetSkill12:onExitChantDo()

end

-- 技能吟唱状态onUpdate时技能操作
function PetSkill12:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function PetSkill12:onEnterProcessDo(state)
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function PetSkill12:onExitProcessDo()

end

-- 技能执行状态onUpdate时技能操作
function PetSkill12:onUpdateProcessDo(dt)

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function PetSkill12:onEnterReleaseDo(state)
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function PetSkill12:onExitReleaseDo()

end

-- 技能释放状态onUpdate时技能操作
function PetSkill12:onUpdateReleaseDo(dt)
