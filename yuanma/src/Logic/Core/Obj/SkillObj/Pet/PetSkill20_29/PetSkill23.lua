...
-- All Rights Reserved.
--==================================================
-- filename:  PetSkill23.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/27
-- descrip:   宠物技能攻击23  玉兔
--===================================================
local PetSkill23 = class("PetSkill23",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function PetSkill23:ctor()
    self._strName = "PetSkill23"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kPetSkill23            -- 技能对象类型
...
-- 创建函数
function PetSkill23:create(master,skillInfo)
    local skill = PetSkill23.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function PetSkill23:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function PetSkill23:onExitPetSkill23()    
    self:onExitSkillObj()
end

-- 循环更新
function PetSkill23:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function PetSkill23:initStateMachine()
...
-- 初始化动作帧事件回调
function PetSkill23:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function PetSkill23:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function PetSkill23:onEnterIdleDo(state)
...
-- 技能待机状态onExit时技能操作
function PetSkill23:onExitIdleDo()

end

-- 技能待机状态onUpdate时技能操作
function PetSkill23:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function PetSkill23:onEnterChantDo(state)
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function PetSkill23:onExitChantDo()

end

-- 技能吟唱状态onUpdate时技能操作
function PetSkill23:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function PetSkill23:onEnterProcessDo(state)
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function PetSkill23:onExitProcessDo()

end

-- 技能执行状态onUpdate时技能操作
function PetSkill23:onUpdateProcessDo(dt)
...
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function PetSkill23:onEnterReleaseDo(state)
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function PetSkill23:onExitReleaseDo()

end

-- 技能释放状态onUpdate时技能操作
function PetSkill23:onUpdateReleaseDo(dt)
