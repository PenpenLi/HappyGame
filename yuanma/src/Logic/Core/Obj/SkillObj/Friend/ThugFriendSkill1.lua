...
-- All Rights Reserved.
--==================================================
-- filename:  ThugFriendSkill1.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/8/14
-- descrip:   好友技能1 【刺客】
--===================================================
local ThugFriendSkill1 = class("ThugFriendSkill1",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function ThugFriendSkill1:ctor()
...
-- 创建函数
function ThugFriendSkill1:create(master, skillInfo)   
    local skill = ThugFriendSkill1.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function ThugFriendSkill1:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function ThugFriendSkill1:onExitThugFriendSkill1()    
    self:onExitSkillObj()
end

-- 循环更新
function ThugFriendSkill1:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function ThugFriendSkill1:initStateMachine()
...
-- 技能使用接口
function ThugFriendSkill1:onUse() 
    -- 立即手动切换到吟唱状态
...
-- 初始化动作帧事件回调
function ThugFriendSkill1:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function ThugFriendSkill1:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function ThugFriendSkill1:onEnterIdleDo(state)
    --print("ThugFriendSkill1:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function ThugFriendSkill1:onExitIdleDo()
--print("ThugFriendSkill1:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function ThugFriendSkill1:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function ThugFriendSkill1:onEnterChantDo(state)
    --print("ThugFriendSkill1:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function ThugFriendSkill1:onExitChantDo()

end

-- 技能吟唱状态onUpdate时技能操作
function ThugFriendSkill1:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function ThugFriendSkill1:onEnterProcessDo(state)
...
-- 技能执行状态onExit时技能操作
function ThugFriendSkill1:onExitProcessDo()
--print("ThugFriendSkill1:onExitProcessDo()")

end

-- 技能执行状态onUpdate时技能操作
function ThugFriendSkill1:onUpdateProcessDo(dt)   

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function ThugFriendSkill1:onEnterReleaseDo(state)
    --print("ThugFriendSkill1:onEnterReleaseDo()")
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function ThugFriendSkill1:onExitReleaseDo()
    --print("ThugFriendSkill1:onExitReleaseDo()")

end

-- 技能释放状态onUpdate时技能操作
function ThugFriendSkill1:onUpdateReleaseDo(dt)

end

---------------------------------------------------------------------------------------------------------
return ThugFriendSkill1
