...
-- All Rights Reserved.
--==================================================
-- filename:  ThugSkill12.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/6/18
-- descrip:   刺客技能12 【嗜血刃 】
--===================================================
local ThugSkill12 = class("ThugSkill12",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function ThugSkill12:ctor()
    self._strName = "ThugSkill12"                              -- 技能名称
    self._kTypeID = kType.kSkill.kID.kThugSkill12              -- 技能对象类型
    self._pCurState = nil                                      -- 技能当前的状态机状态
    
    self._bStickAdd = false                                    -- 引用计数的标记
    self._bIgnoreHurtAdd = false                               -- 引用计数的标记
    
    self._fShootActDuration = 1                                -- 冲刺的持续时间
...
-- 创建函数
function ThugSkill12:create(master, skillInfo)   
    local skill = ThugSkill12.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function ThugSkill12:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function ThugSkill12:onExitThugSkill12()    
    self:onExitSkillObj()
end

-- 循环更新
function ThugSkill12:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function ThugSkill12:initStateMachine()
...
-- 初始化动作帧事件回调
function ThugSkill12:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function ThugSkill12:procActionsFrameEvents()
...
function ThugSkill12:AttackOffset()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function ThugSkill12:onEnterIdleDo(state)
    --print("ThugSkill12:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function ThugSkill12:onExitIdleDo()
--print("ThugSkill12:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function ThugSkill12:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function ThugSkill12:onEnterChantDo(state)
    --print("ThugSkill12:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function ThugSkill12:onExitChantDo()
--print("ThugSkill12:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function ThugSkill12:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function ThugSkill12:onEnterProcessDo(state)
    --print("ThugSkill12:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function ThugSkill12:onExitProcessDo()
--print("ThugSkill12:onExitProcessDo()")

end

-- 技能执行状态onUpdate时技能操作
function ThugSkill12:onUpdateProcessDo(dt)   

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function ThugSkill12:onEnterReleaseDo(state)
    --print("ThugSkill12:onEnterReleaseDo()")
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function ThugSkill12:onExitReleaseDo()
    --print("ThugSkill12:onExitReleaseDo()")
    -- 忽略伤害引用计数-1（连应值都不会有）
