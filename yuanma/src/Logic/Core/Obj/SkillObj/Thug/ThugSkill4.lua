...
-- All Rights Reserved.
--==================================================
-- filename:  ThugSkill4.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/4/15
-- descrip:   刺客技能4 【风火雷】
--===================================================
local ThugSkill4 = class("ThugSkill4",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function ThugSkill4:ctor()
    self._strName = "ThugSkill4"                               -- 技能名称
    self._kTypeID = kType.kSkill.kID.kThugSkill4               -- 技能对象类型
...
-- 创建函数
function ThugSkill4:create(master, skillInfo)   
    local skill = ThugSkill4.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function ThugSkill4:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function ThugSkill4:onExitThugSkill4()    
    self:onExitSkillObj()
end

-- 循环更新
function ThugSkill4:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function ThugSkill4:initStateMachine()
...
-- 初始化动作帧事件回调
function ThugSkill4:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function ThugSkill4:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function ThugSkill4:onEnterIdleDo(state)
    --print("ThugSkill4:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function ThugSkill4:onExitIdleDo()
--print("ThugSkill4:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function ThugSkill4:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function ThugSkill4:onEnterChantDo(state)
    --print("ThugSkill4:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function ThugSkill4:onExitChantDo()
--print("ThugSkill4:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function ThugSkill4:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function ThugSkill4:onEnterProcessDo(state)
    --print("ThugSkill4:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function ThugSkill4:onExitProcessDo()
--print("ThugSkill4:onExitProcessDo()")

end

-- 技能执行状态onUpdate时技能操作
function ThugSkill4:onUpdateProcessDo(dt)   

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function ThugSkill4:onEnterReleaseDo(state)
    --print("ThugSkill4:onEnterReleaseDo()")
    self._pCurState = state

end

-- 技能释放状态onExit时技能操作
function ThugSkill4:onExitReleaseDo()
    --print("ThugSkill4:onExitReleaseDo()")

end

-- 技能释放状态onUpdate时技能操作
function ThugSkill4:onUpdateReleaseDo(dt)

end

-- 技能结束时的复位操作
function ThugSkill4:reset()
    -- 复位给技能指定施展时的zorder
