...
-- All Rights Reserved.
--==================================================
-- filename:  MonsterSkill44.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/22
-- descrip:   怪物技能攻击44  蚩尤
--===================================================
local MonsterSkill44 = class("MonsterSkill44",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MonsterSkill44:ctor()
    self._strName = "MonsterSkill44"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMonsterSkill44            -- 技能对象类型
...
-- 创建函数
function MonsterSkill44:create(master,skillInfo)
    local skill = MonsterSkill44.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MonsterSkill44:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MonsterSkill44:onExitMonsterSkill44()    
    self:onExitSkillObj()
end

-- 循环更新
function MonsterSkill44:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MonsterSkill44:initStateMachine()
...
-- 初始化动作帧事件回调
function MonsterSkill44:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MonsterSkill44:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MonsterSkill44:onEnterIdleDo(state)
...
-- 技能待机状态onExit时技能操作
function MonsterSkill44:onExitIdleDo()

end

-- 技能待机状态onUpdate时技能操作
function MonsterSkill44:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MonsterSkill44:onEnterChantDo(state)
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MonsterSkill44:onExitChantDo()

end

-- 技能吟唱状态onUpdate时技能操作
function MonsterSkill44:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MonsterSkill44:onEnterProcessDo(state)
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MonsterSkill44:onExitProcessDo()

end

-- 技能执行状态onUpdate时技能操作
function MonsterSkill44:onUpdateProcessDo(dt)
...
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MonsterSkill44:onEnterReleaseDo(state)
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function MonsterSkill44:onExitReleaseDo()

end

-- 技能释放状态onUpdate时技能操作
function MonsterSkill44:onUpdateReleaseDo(dt)

end

---------------------------------------------------------------------------------------------------------
return MonsterSkill44
