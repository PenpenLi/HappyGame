...
-- All Rights Reserved.
--==================================================
-- filename:  MonsterSkill40.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/22
-- descrip:   怪物技能攻击40 妲己
--===================================================
local MonsterSkill40 = class("MonsterSkill40",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MonsterSkill40:ctor()
    self._strName = "MonsterSkill40"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMonsterSkill40            -- 技能对象类型
...
-- 创建函数
function MonsterSkill40:create(master,skillInfo)
    local skill = MonsterSkill40.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MonsterSkill40:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MonsterSkill40:onExitMonsterSkill40()    
    self:onExitSkillObj()
end

-- 循环更新
function MonsterSkill40:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MonsterSkill40:initStateMachine()
...
-- 初始化动作帧事件回调
function MonsterSkill40:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MonsterSkill40:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MonsterSkill40:onEnterIdleDo(state)
...
-- 技能待机状态onExit时技能操作
function MonsterSkill40:onExitIdleDo()

end

-- 技能待机状态onUpdate时技能操作
function MonsterSkill40:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MonsterSkill40:onEnterChantDo(state)
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MonsterSkill40:onExitChantDo()

end

-- 技能吟唱状态onUpdate时技能操作
function MonsterSkill40:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MonsterSkill40:onEnterProcessDo(state)
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MonsterSkill40:onExitProcessDo()

end

-- 技能执行状态onUpdate时技能操作
function MonsterSkill40:onUpdateProcessDo(dt)
...
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MonsterSkill40:onEnterReleaseDo(state)
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function MonsterSkill40:onExitReleaseDo()

end

-- 技能释放状态onUpdate时技能操作
function MonsterSkill40:onUpdateReleaseDo(dt)

end

---------------------------------------------------------------------------------------------------------
return MonsterSkill40
