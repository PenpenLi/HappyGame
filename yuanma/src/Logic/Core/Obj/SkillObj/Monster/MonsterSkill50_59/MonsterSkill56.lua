...
-- All Rights Reserved.
--==================================================
-- filename:  MonsterSkill56.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/23
-- descrip:   怪物技能攻击56  饕餮
--===================================================
local MonsterSkill56 = class("MonsterSkill56",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MonsterSkill56:ctor()
    self._strName = "MonsterSkill56"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMonsterSkill56            -- 技能对象类型
    self._pCurState = nil                                       -- 技能当前的状态机状态
    
    self._nRoleAttackActionIndex = 3                            -- 角色攻击动作index
    self._fChantDelayTime = 0.48                                 -- 吟唱动作持续时间s
    
end

-- 创建函数
function MonsterSkill56:create(master,skillInfo)
    local skill = MonsterSkill56.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MonsterSkill56:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MonsterSkill56:onExitMonsterSkill56()    
    self:onExitSkillObj()
end

-- 循环更新
function MonsterSkill56:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MonsterSkill56:initStateMachine()
...
-- 初始化动作帧事件回调
function MonsterSkill56:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MonsterSkill56:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MonsterSkill56:onEnterIdleDo(state)
    --print("MonsterSkill56:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function MonsterSkill56:onExitIdleDo()
--print("MonsterSkill56:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function MonsterSkill56:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MonsterSkill56:onEnterChantDo(state)
    --print("MonsterSkill56:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MonsterSkill56:onExitChantDo()
--print("MonsterSkill56:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function MonsterSkill56:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MonsterSkill56:onEnterProcessDo(state)
    --print("MonsterSkill56:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MonsterSkill56:onExitProcessDo()
--print("MonsterSkill56:onExitProcessDo()")
end

-- 技能执行状态onUpdate时技能操作
function MonsterSkill56:onUpdateProcessDo(dt)

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MonsterSkill56:onEnterReleaseDo(state)
    --print("MonsterSkill56:onEnterReleaseDo()")
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function MonsterSkill56:onExitReleaseDo()
--print("MonsterSkill56:onExitReleaseDo()")

end

-- 技能释放状态onUpdate时技能操作
function MonsterSkill56:onUpdateReleaseDo(dt)

end

---------------------------------------------------------------------------------------------------------
return MonsterSkill56
