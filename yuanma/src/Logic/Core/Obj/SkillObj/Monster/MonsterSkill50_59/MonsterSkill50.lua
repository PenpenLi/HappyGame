...
-- All Rights Reserved.
--==================================================
-- filename:  MonsterSkill50.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/23
-- descrip:   怪物技能攻击50  章鱼
--===================================================
local MonsterSkill50 = class("MonsterSkill50",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MonsterSkill50:ctor()
    self._strName = "MonsterSkill50"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMonsterSkill50            -- 技能对象类型
...
-- 创建函数
function MonsterSkill50:create(master,skillInfo)
    local skill = MonsterSkill50.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MonsterSkill50:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MonsterSkill50:onExitMonsterSkill50()    
    self:onExitSkillObj()
end

-- 循环更新
function MonsterSkill50:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MonsterSkill50:initStateMachine()
...
-- 初始化动作帧事件回调
function MonsterSkill50:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MonsterSkill50:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MonsterSkill50:onEnterIdleDo(state)
    --print("MonsterSkill50:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function MonsterSkill50:onExitIdleDo()
--print("MonsterSkill50:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function MonsterSkill50:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MonsterSkill50:onEnterChantDo(state)
    --print("MonsterSkill50:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MonsterSkill50:onExitChantDo()
--print("MonsterSkill50:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function MonsterSkill50:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MonsterSkill50:onEnterProcessDo(state)
    --print("MonsterSkill50:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MonsterSkill50:onExitProcessDo()
--print("MonsterSkill50:onExitProcessDo()")
end

-- 技能执行状态onUpdate时技能操作
function MonsterSkill50:onUpdateProcessDo(dt)

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MonsterSkill50:onEnterReleaseDo(state)
    --print("MonsterSkill50:onEnterReleaseDo()")
    self._pCurState = state
    ...
    if self._pSkillInfo.EarlyWarningType == kType.kSkillEarlyWarning.kType3 then
        ...
        if table.getn(tTargets) == 0 then
            ...
            self._posTargetsPos = cc.p(tTargets[1].enemy:getPositionX(),tTargets[index].enemy:getPositionY())
        end
    end
...
-- 技能释放状态onExit时技能操作
function MonsterSkill50:onExitReleaseDo()
--print("MonsterSkill50:onExitReleaseDo()")

end

-- 技能释放状态onUpdate时技能操作
function MonsterSkill50:onUpdateReleaseDo(dt)

end

---------------------------------------------------------------------------------------------------------
return MonsterSkill50
