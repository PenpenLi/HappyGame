...
-- All Rights Reserved.
--==================================================
-- filename:  MonsterSkill19.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/20
-- descrip:   怪物技能攻击19
--===================================================
local MonsterSkill19 = class("MonsterSkill19",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MonsterSkill19:ctor()
    self._strName = "MonsterSkill19"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMonsterSkill19            -- 技能对象类型
...
-- 创建函数
function MonsterSkill19:create(master,skillInfo)
    local skill = MonsterSkill19.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MonsterSkill19:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MonsterSkill19:onExitMonsterSkill19()    
    self:onExitSkillObj()
end

-- 循环更新
function MonsterSkill19:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MonsterSkill19:initStateMachine()
...
-- 初始化动作帧事件回调
function MonsterSkill19:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MonsterSkill19:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MonsterSkill19:onEnterIdleDo(state)
    --print("MonsterSkill19:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function MonsterSkill19:onExitIdleDo()
--print("MonsterSkill19:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function MonsterSkill19:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MonsterSkill19:onEnterChantDo(state)
    --print("MonsterSkill19:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MonsterSkill19:onExitChantDo()
--print("MonsterSkill19:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function MonsterSkill19:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MonsterSkill19:onEnterProcessDo(state)
    --print("MonsterSkill19:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MonsterSkill19:onExitProcessDo()
--print("MonsterSkill19:onExitProcessDo()")
end

-- 技能执行状态onUpdate时技能操作
function MonsterSkill19:onUpdateProcessDo(dt)

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MonsterSkill19:onEnterReleaseDo(state)
    --print("MonsterSkill19:onEnterReleaseDo()")
    self._pCurState = state
    ...
    if self._pSkillInfo.EarlyWarningType == kType.kSkillEarlyWarning.kType3 then  -- 大号箭头
        ...
        self._posTargetsPos = cc.p(self:getMaster():getPositionX()+offsetX, self:getMaster():getPositionY()+self:getMaster():getHeight()/2)
...
-- 技能释放状态onExit时技能操作
function MonsterSkill19:onExitReleaseDo()
--print("MonsterSkill19:onExitReleaseDo()")

end

-- 技能释放状态onUpdate时技能操作
function MonsterSkill19:onUpdateReleaseDo(dt)

end

---------------------------------------------------------------------------------------------------------
return MonsterSkill19
