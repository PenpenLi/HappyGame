...
-- All Rights Reserved.
--==================================================
-- filename:  MonsterSkill24.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/7/20
-- descrip:   怪物技能攻击24 （阴兵子物体）
--===================================================
local MonsterSkill24 = class("MonsterSkill24",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MonsterSkill24:ctor()
    self._strName = "MonsterSkill24"                            -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMonsterSkill24            -- 技能对象类型
...
-- 创建函数
function MonsterSkill24:create(master,skillInfo)
    local skill = MonsterSkill24.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MonsterSkill24:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MonsterSkill24:onExitMonsterSkill24()    
    self:onExitSkillObj()
end

-- 循环更新
function MonsterSkill24:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MonsterSkill24:initStateMachine()
...
-- 初始化动作帧事件回调
function MonsterSkill24:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MonsterSkill24:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MonsterSkill24:onEnterIdleDo(state)
...
-- 技能待机状态onExit时技能操作
function MonsterSkill24:onExitIdleDo()

end

-- 技能待机状态onUpdate时技能操作
function MonsterSkill24:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MonsterSkill24:onEnterChantDo(state)
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MonsterSkill24:onExitChantDo()

end

-- 技能吟唱状态onUpdate时技能操作
function MonsterSkill24:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MonsterSkill24:onEnterProcessDo(state)
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MonsterSkill24:onExitProcessDo()

end

-- 技能执行状态onUpdate时技能操作
function MonsterSkill24:onUpdateProcessDo(dt)
...
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MonsterSkill24:onEnterReleaseDo(state)
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function MonsterSkill24:onExitReleaseDo()

end

-- 技能释放状态onUpdate时技能操作
function MonsterSkill24:onUpdateReleaseDo(dt)

end

---------------------------------------------------------------------------------------------------------
return MonsterSkill24
