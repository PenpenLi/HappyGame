...
-- All Rights Reserved.
--==================================================
-- filename:  WarriorSkill6.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/6/8
-- descrip:   战士技能6 【奔雷斩 】
--===================================================
local WarriorSkill6 = class("WarriorSkill6",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function WarriorSkill6:ctor()
    self._strName = "WarriorSkill6"                               -- 技能名称
    self._kTypeID = kType.kSkill.kID.kWarriorSkill6               -- 技能对象类型
...
-- 创建函数
function WarriorSkill6:create(master, skillInfo)   
    local skill = WarriorSkill6.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function WarriorSkill6:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function WarriorSkill6:onExitWarriorSkill6()    
    self:onExitSkillObj()
end

-- 循环更新
function WarriorSkill6:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function WarriorSkill6:initStateMachine()
...
-- 初始化动作帧事件回调
function WarriorSkill6:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function WarriorSkill6:procActionsFrameEvents()
    ...
    elseif self._strFrameEventName == "hurt1_1" then
        ...
        for i=-3,rectNum do
            local rectTmp = cc.rect(self:getPositionX() + offsetX*i/(rectNum+3), self:getPositionY() + offsetY*i/(rectNum+3),20,20)
...
function WarriorSkill6:AttackOffset()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function WarriorSkill6:onEnterIdleDo(state)
    --print("WarriorSkill6:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function WarriorSkill6:onExitIdleDo()
--print("WarriorSkill6:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function WarriorSkill6:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function WarriorSkill6:onEnterChantDo(state)
    --print("WarriorSkill6:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function WarriorSkill6:onExitChantDo()
--print("WarriorSkill6:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function WarriorSkill6:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function WarriorSkill6:onEnterProcessDo(state)
    --print("WarriorSkill6:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function WarriorSkill6:onExitProcessDo()
--print("WarriorSkill6:onExitProcessDo()")

end

-- 技能执行状态onUpdate时技能操作
function WarriorSkill6:onUpdateProcessDo(dt)   

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function WarriorSkill6:onEnterReleaseDo(state)
    --print("WarriorSkill6:onEnterReleaseDo()")
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function WarriorSkill6:onExitReleaseDo()
    --print("WarriorSkill6:onExitReleaseDo()")
    -- 忽略伤害引用计数-1（连应值都不会有）
...
-- 技能释放状态onUpdate时技能操作
function WarriorSkill6:onUpdateReleaseDo(dt)

end

-- 技能结束时的复位操作
function WarriorSkill6:reset()
    -- 复位给技能指定施展时的zorder
