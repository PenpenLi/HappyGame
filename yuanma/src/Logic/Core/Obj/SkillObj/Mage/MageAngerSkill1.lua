...
-- All Rights Reserved.
--==================================================
-- filename:  MageAngerSkill1.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/5/22
-- descrip:   人鱼怒气技能技能1 青龙斩
--===================================================
local MageAngerSkill1 = class("MageAngerSkill1",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MageAngerSkill1:ctor()
    self._strName = "MageAngerSkill1"                          -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMageAngerSkill1          -- 技能对象类型
...
-- 创建函数
function MageAngerSkill1:create(master, skillInfo)   
    local skill = MageAngerSkill1.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MageAngerSkill1:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MageAngerSkill1:onExitMageAngerSkill1()    
    self:onExitSkillObj()
end

-- 循环更新
function MageAngerSkill1:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MageAngerSkill1:initStateMachine()
...
-- 技能使用接口
function MageAngerSkill1:onUse() 
    -- test
...
-- 初始化动作帧事件回调
function MageAngerSkill1:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MageAngerSkill1:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MageAngerSkill1:onEnterIdleDo(state)
    --print("MageAngerSkill1:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function MageAngerSkill1:onExitIdleDo()
--print("MageAngerSkill1:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function MageAngerSkill1:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MageAngerSkill1:onEnterChantDo(state)
    --print("MageAngerSkill1:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MageAngerSkill1:onExitChantDo()
--print("MageAngerSkill1:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function MageAngerSkill1:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MageAngerSkill1:onEnterProcessDo(state)
    --print("MageAngerSkill1:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MageAngerSkill1:onExitProcessDo()
--print("MageAngerSkill1:onExitProcessDo()")

end

-- 技能执行状态onUpdate时技能操作
function MageAngerSkill1:onUpdateProcessDo(dt)   

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MageAngerSkill1:onEnterReleaseDo(state)
    --print("MageAngerSkill1:onEnterReleaseDo()")
    self._pCurState = state
...
-- 技能释放状态onExit时技能操作
function MageAngerSkill1:onExitReleaseDo()
    --print("MageAngerSkill1:onExitReleaseDo()")
...
-- 技能释放状态onUpdate时技能操作
function MageAngerSkill1:onUpdateReleaseDo(dt)

end

-- 技能结束时的复位操作
function MageAngerSkill1:reset()
    -- 复位给技能指定施展时的zorder
