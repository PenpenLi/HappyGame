...
-- All Rights Reserved.
--==================================================
-- filename:  MageSkill7.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/5/12
-- descrip:   人鱼技能7 【大雷锤】
--===================================================
local MageSkill7 = class("MageSkill7",function(master, skillInfo)
    return require("SkillObj"):create(master, skillInfo)
end)

-- 构造函数
function MageSkill7:ctor()
    self._strName = "MageSkill7"                               -- 技能名称
    self._kTypeID = kType.kSkill.kID.kMageSkill7               -- 技能对象类型
...
-- 创建函数
function MageSkill7:create(master, skillInfo)   
    local skill = MageSkill7.new(master, skillInfo)
    skill:dispose()
    return skill
end

-- 处理函数
function MageSkill7:dispose()
    ------------------- 初始化 ------------------------  
    -- 创建状态机
...
-- 退出函数
function MageSkill7:onExitMageSkill7()    
    self:onExitSkillObj()
end

-- 循环更新
function MageSkill7:update(dt)
    self:updateSkillObj(dt)
end

-- 创建状态机
function MageSkill7:initStateMachine()
...
-- 初始化动作帧事件回调
function MageSkill7:initActionsFrameEvents(index, action)
    if index == 1 then
...
-- 帧事件的处理
function MageSkill7:procActionsFrameEvents()
...
-------------------------------------------------------------------------------------------------
-- 技能待机状态onEnter时技能操作
function MageSkill7:onEnterIdleDo(state)
    --print("MageSkill7:onEnterIdleDo()")
...
-- 技能待机状态onExit时技能操作
function MageSkill7:onExitIdleDo()
--print("MageSkill7:onExitIdleDo()")
end

-- 技能待机状态onUpdate时技能操作
function MageSkill7:onUpdateIdleDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能吟唱状态onEnter时技能操作
function MageSkill7:onEnterChantDo(state)
    --print("MageSkill7:onEnterChantDo()")
    self._pCurState = state
...
-- 技能吟唱状态onExit时技能操作
function MageSkill7:onExitChantDo()
--print("MageSkill7:onExitChantDo()")
end

-- 技能吟唱状态onUpdate时技能操作
function MageSkill7:onUpdateChantDo(dt)

end
-------------------------------------------------------------------------------------------------
-- 技能执行状态onEnter时技能操作
function MageSkill7:onEnterProcessDo(state)
    --print("MageSkill7:onEnterProcessDo()")
    self._pCurState = state
...
-- 技能执行状态onExit时技能操作
function MageSkill7:onExitProcessDo()
--print("MageSkill7:onExitProcessDo()")

end

-- 技能执行状态onUpdate时技能操作
function MageSkill7:onUpdateProcessDo(dt)   

end
-----------------------------------------------------------------------------------------------------
-- 技能释放状态onEnter时技能操作
function MageSkill7:onEnterReleaseDo(state)
    --print("MageSkill7:onEnterReleaseDo()")
    self._pCurState = state

end

-- 技能释放状态onExit时技能操作
function MageSkill7:onExitReleaseDo()
    --print("MageSkill7:onExitReleaseDo()")

end

-- 技能释放状态onUpdate时技能操作
function MageSkill7:onUpdateReleaseDo(dt)

end

-- 技能结束时的复位操作
function MageSkill7:reset()
    -- 复位给技能指定施展时的zorder
