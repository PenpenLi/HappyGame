...
-- All Rights Reserved.
--==================================================
-- filename:  BattleOtherPlayerRoleStateMachine.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/10/8
-- descrip:   战斗中其他玩家角色状态机
--===================================================
local BattleOtherPlayerRoleStateMachine = class("BattleOtherPlayerRoleStateMachine",function()
    return require("StateMachine"):create()
end)

-- 构造函数
function BattleOtherPlayerRoleStateMachine:ctor()
    self._strName = "BattleOtherPlayerRoleStateMachine"         -- 状态机名称
    self._kTypeID = kType.kStateMachine.kBattleOtherPlayerRole  -- 状态类机型ID
...
-- 创建函数
function BattleOtherPlayerRoleStateMachine:create(master)
    local machine = BattleOtherPlayerRoleStateMachine.new()
    machine:onEnter(master)
    return machine
end

-- 进入函数
function BattleOtherPlayerRoleStateMachine:onEnter(master)
    --print(self._strName.." is onEnter!")
    self:setMaster(master)
    
    self:addState(require("BattleOtherPlayerRoleAppearState"):create())      -- 加入出场状态到状态机
    self:addState(require("BattleOtherPlayerRoleStandState"):create())       -- 加入站立状态到状态机
    self:addState(require("BattleOtherPlayerRoleRunState"):create())         -- 加入奔跑状态到状态机
    self:addState(require("BattleOtherPlayerRoleGenAttackState"):create())   -- 加入普通攻击状态到状态机
    self:addState(require("BattleOtherPlayerRoleBebeatedState"):create())    -- 加入受击状态到状态机
    self:addState(require("BattleOtherPlayerRoleDeadState"):create())        -- 加入死亡状态到状态机
    self:addState(require("BattleOtherPlayerRoleAngerAttackState"):create()) -- 加入怒气技能状态到状态机
    self:addState(require("BattleOtherPlayerRoleDizzyState"):create())       -- 加入眩晕状态到状态机
    self:addState(require("BattleOtherPlayerRoleFrozenState"):create())      -- 加入冻结状态到状态机
    self:addState(require("BattleOtherPlayerRoleSkillAttackState"):create()) -- 加入技能状态到状态机
    
    self:setCurStateByTypeID(kType.kState.kBattleOtherPlayerRole.kAppear, true)  -- 设置当前状态为出场状态
    
    return
end

-- 退出函数
function BattleOtherPlayerRoleStateMachine:onExit()
...
-- 更新逻辑
function BattleOtherPlayerRoleStateMachine:update(dt)    
    if self._pCurState ~= nil then
        self._pCurState:update(dt)
    end

    if self:getMaster() then
        if self._pCurState._kTypeID ~= kType.kState.kBattleOtherPlayerRole.kDead and self:getMaster()._nCurHp > 0 then
...
-- 使用共鸣技能
function BattleOtherPlayerRoleStateMachine:usePetCooperateSkill()
    if self:getMaster()._pCurPetRole and self:getMaster()._pCurPetRole._pCooperateInfo and self:getMaster()._pCurPetRole._pCooperateInfo.CD and self:getMaster()._pCurPetRole._nCurHp > 0 then  -- 存在共鸣
        if self._nPetCooperateCDCounter >= self:getMaster()._pCurPetRole._pCooperateInfo.CD then
            self._nPetCooperateCDCounter = 0
            AIManager:getInstance():usePetCooperateSkillByCampType(kType.kCampType.kOther,self:getMaster())
            cclog("其他玩家释放宠物共鸣")
        end
    end
end

return BattleOtherPlayerRoleStateMachine
