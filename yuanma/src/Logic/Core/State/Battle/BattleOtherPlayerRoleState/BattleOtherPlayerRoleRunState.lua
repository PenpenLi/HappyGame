...
-- All Rights Reserved.
--==================================================
-- filename:  BattleOtherPlayerRoleRunState.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/10/8
-- descrip:   战斗中其他玩家角色奔跑状态
--===================================================
local BattleOtherPlayerRoleRunState = class("BattleOtherPlayerRoleRunState",function()
    return require("State"):create()
end)

-- 构造函数
function BattleOtherPlayerRoleRunState:ctor()
    self._strName = "BattleOtherPlayerRoleRunState"           -- 状态名称
    self._kTypeID = kType.kState.kBattleOtherPlayerRole.kRun  -- 状态类型ID
...
-- 创建函数
function BattleOtherPlayerRoleRunState:create()
    local state = BattleOtherPlayerRoleRunState.new()
    return state
end

-- 进入函数
function BattleOtherPlayerRoleRunState:onEnter(args)
...
-- 退出函数
function BattleOtherPlayerRoleRunState:onExit()
...
-- 更新逻辑
function BattleOtherPlayerRoleRunState:update(dt)
...
-- 自动战斗相关逻辑
function BattleOtherPlayerRoleRunState:procAutoBattle(dt)
    ...
    elseif #targetsInView ~= 0 then  -- 警戒范围内没有目标，视野范围内有目标，则开始寻路
        if self._fRunTimeCount >= 2.0 then  -- 超过2秒，重新寻路
            ...
            self:getMaster():getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPlayerRole):setCurStateByTypeID(kType.kState.kBattleOtherPlayerRole.kRun, true, {moveDirections = path})
            return
        end
    end
...
-- 奔跑逻辑
function BattleOtherPlayerRoleRunState:procRun(dt)      
...
return BattleOtherPlayerRoleRunState
