...
-- All Rights Reserved.
--==================================================
-- filename:  BattlePlayerRoleAngerAttackState.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/3/30
-- descrip:   战斗中玩家角色怒气技能攻击状态
--===================================================
local BattlePlayerRoleAngerAttackState = class("BattlePlayerRoleAngerAttackState",function()
    return require("State"):create()
end)

-- 构造函数
function BattlePlayerRoleAngerAttackState:ctor()
    self._strName = "BattlePlayerRoleAngerAttackState"           -- 状态名称
    self._kTypeID = kType.kState.kBattlePlayerRole.kAngerAttack  -- 状态类型ID

end

-- 创建函数
function BattlePlayerRoleAngerAttackState:create()
    local state = BattlePlayerRoleAngerAttackState.new()
    return state
end

-- 进入函数
function BattlePlayerRoleAngerAttackState:onEnter(args)
...
-- 退出函数
function BattlePlayerRoleAngerAttackState:onExit()
    if self:getMaster() then
...
-- 更新逻辑
function BattlePlayerRoleAngerAttackState:update(dt)
    if self:getMaster() then
        -- 检测遮挡
...
return BattlePlayerRoleAngerAttackState
