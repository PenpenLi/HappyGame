...
-- All Rights Reserved.
--==================================================
-- filename:  BattlePetRoleStandState.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/4/24
-- descrip:   战斗中玩家宠物角色站立状态
--===================================================
local BattlePetRoleStandState = class("BattlePetRoleStandState",function()
    return require("State"):create()
end)

-- 构造函数
function BattlePetRoleStandState:ctor()
    self._strName = "BattlePetRoleStandState"           -- 状态名称
    self._kTypeID = kType.kState.kBattlePetRole.kStand  -- 状态类型ID
...
-- 创建函数
function BattlePetRoleStandState:create()
    local state = BattlePetRoleStandState.new()
    return state
end
...
-- 退出函数
function BattlePetRoleStandState:onExit()
    --print(self._strName.." is onExit!")
    return
end

-- 更新逻辑
function BattlePetRoleStandState:update(dt)
    if self:getMaster() then
...
-- 检查位置相关逻辑
function BattlePetRoleStandState:procCheckPos(dt)
    -- 时间计数
    ...
    if self._fTimeCounter >= 0.5 then   -- 每隔0.5s检测一次
        ...
        if math.abs(posIndex.x - targetPosIndex.x) > 4 or math.abs(posIndex.y - targetPosIndex.y) > 4 then      -- 横竖超过4个格子时需要寻路跟随
            ...
            self:getMaster():getStateMachineByTypeID(kType.kStateMachine.kBattlePetRole):setCurStateByTypeID(kType.kState.kBattlePetRole.kRun, false, {moveDirections = path, bExictlyToTarget = false})  -- 不用精确走到目标位置（用于跟随主人）
        end
...
-- 自动战斗相关逻辑
function BattlePetRoleStandState:procAutoBattle(dt)
    ...
    if self:getMaster()._strCharTag == "main" then
        ...
        -- 测试使用
        --self:getMaster():getStateMachineByTypeID(kType.kStateMachine.kBattlePetRole):setCurStateByTypeID(kType.kState.kBattlePetRole.kSkillAttack, true, {["skillIndex"] = 1})
               ...
               if vSkill._pSkillInfo.PetSkillType == 4 then -- 回复性技能类型
                    if vSkill:isCDOver() == true then   -- 且已经CD结束，则使用该回复技能
                        ...
                        if #targetInWarning ~= 0 then  -- 警戒范围内有目标，则直接开始战斗
                            self:getMaster():getStateMachineByTypeID(kType.kStateMachine.kBattlePetRole):setCurStateByTypeID(kType.kState.kBattlePetRole.kSkillAttack, false, {["skillIndex"] = kSkill})
                            return 
                        end
                    end
               end
        ...
        if damageSkillCDOverNum ~= 0 then
            ...
            if #targetInView ~= 0 then  -- 视野范围内有目标，则开始寻路
                ...
                if posIndex.x ~= targetPosIndex.x or posIndex.y ~= targetPosIndex.y then
                    local path = mmo.AStarHelper:getInst():ComputeAStar(posIndex, targetPosIndex)
                    self:getMaster():getStateMachineByTypeID(kType.kStateMachine.kBattlePetRole):setCurStateByTypeID(kType.kState.kBattlePetRole.kRun, false, {moveDirections = path, bExictlyToTarget = true})  -- 追怪
                end
                return 
            end
...
return BattlePetRoleStandState
