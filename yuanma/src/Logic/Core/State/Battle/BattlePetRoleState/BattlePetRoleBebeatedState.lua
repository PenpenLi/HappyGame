...
-- All Rights Reserved.
--==================================================
-- filename:  BattlePetRoleBebeatedState.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/4/24
-- descrip:   战斗中宠物角色受击状态
--===================================================
local BattlePetRoleBebeatedState = class("BattlePetRoleBebeatedState",function()
    return require("State"):create()
end)

-- 构造函数
function BattlePetRoleBebeatedState:ctor()
    self._strName = "BattlePetRoleBebeatedState"           -- 状态名称
    self._kTypeID = kType.kState.kBattlePetRole.kBeaten    -- 状态类型ID
...
-- 创建函数
function BattlePetRoleBebeatedState:create()
    local state = BattlePetRoleBebeatedState.new()
    return state
end

-- 进入函数
-- 参数：args[1]：攻击者技能对象    args[2]：（攻击等级-防御等级）
function BattlePetRoleBebeatedState:onEnter(args)
...
-- 退出函数
function BattlePetRoleBebeatedState:onExit()
...
-- 更新逻辑
function BattlePetRoleBebeatedState:update(dt)
    if self:getMaster() then
        -- 检测遮挡
        self:getMaster():checkCover()
        -- 检测标记位，进行状态切换
...
-- Proc受击瞬间的方向刷新
function BattlePetRoleBebeatedState:refreshBeatenDirection()
    if self:getMaster() then
...
-- Proc原地受击
function BattlePetRoleBebeatedState:beatenNoOffset()
    if self:getMaster() then
        -- 刷新新的方向
...
-- Proc被击退
function BattlePetRoleBebeatedState:beatenBack()
    if self:getMaster() then
        -- 刷新新的方向
        self:refreshBeatenDirection()
        -- 播放受击动画
...
-- Proc被击倒
function BattlePetRoleBebeatedState:beatenFall()
    if self:getMaster() then
        -- 刷新新的方向
        self:refreshBeatenDirection()
        -- 播放受击动画
...
-- Proc被击退同时被击倒
function BattlePetRoleBebeatedState:beatenBackAndFall()
    if self:getMaster() then
        -- 刷新新的方向
        self:refreshBeatenDirection()
        -- 播放受击动画
...
-- 反方向后退位移
function BattlePetRoleBebeatedState:toBackDistanceWithTime(time)
    if self:getMaster() then
...
return BattlePetRoleBebeatedState
