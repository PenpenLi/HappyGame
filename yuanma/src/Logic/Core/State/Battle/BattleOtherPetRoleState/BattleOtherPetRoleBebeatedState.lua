...
-- All Rights Reserved.
--==================================================
-- filename:  BattleOtherPetRoleBebeatedState.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/10/10
-- descrip:   战斗中其他玩家宠物角色受击状态
--===================================================
local BattleOtherPetRoleBebeatedState = class("BattleOtherPetRoleBebeatedState",function()
    return require("State"):create()
end)

-- 构造函数
function BattleOtherPetRoleBebeatedState:ctor()
    self._strName = "BattleOtherPetRoleBebeatedState"           -- 状态名称
    self._kTypeID = kType.kState.kBattleOtherPetRole.kBeaten    -- 状态类型ID
...
-- 创建函数
function BattleOtherPetRoleBebeatedState:create()
    local state = BattleOtherPetRoleBebeatedState.new()
    return state
end

-- 进入函数
-- 参数：args[1]：攻击者技能对象    args[2]：（攻击等级-防御等级）
function BattleOtherPetRoleBebeatedState:onEnter(args)
    if self:getMaster() then
        ...
        -- 设置受击类型
        self._kBeatenType = TableTempleteAttackLevelDiff[nAttackAndDefDiff].PetsBeaten[2]
        -- 击退位移
        self._fBeatenOffset = TableTempleteAttackLevelDiff[nAttackAndDefDiff].PetsBeaten[1]
        -- 设置受击角度
...
-- 退出函数
function BattleOtherPetRoleBebeatedState:onExit()
...
-- 更新逻辑
function BattleOtherPetRoleBebeatedState:update(dt)
    if self:getMaster() then
        -- 检测遮挡
        self:getMaster():checkCover()
        -- 检测标记位，进行状态切换
...
-- Proc受击瞬间的方向刷新
function BattleOtherPetRoleBebeatedState:refreshBeatenDirection()
    if self:getMaster() then
...
-- Proc原地受击
function BattleOtherPetRoleBebeatedState:beatenNoOffset()
    if self:getMaster() then
        -- 刷新新的方向
...
-- Proc被击退
function BattleOtherPetRoleBebeatedState:beatenBack()
    if self:getMaster() then
        -- 刷新新的方向
        self:refreshBeatenDirection()
        -- 播放受击动画
...
-- Proc被击倒
function BattleOtherPetRoleBebeatedState:beatenFall()
    if self:getMaster() then
        -- 刷新新的方向
        self:refreshBeatenDirection()
        -- 播放受击动画
...
-- Proc被击退同时被击倒
function BattleOtherPetRoleBebeatedState:beatenBackAndFall()
    if self:getMaster() then
        -- 刷新新的方向
        self:refreshBeatenDirection()
        -- 播放受击动画
...
-- 反方向后退位移
function BattleOtherPetRoleBebeatedState:toBackDistanceWithTime(time)
    if self:getMaster() then
...
return BattleOtherPetRoleBebeatedState
