...
-- All Rights Reserved.
--==================================================
-- filename:  BattleOtherPetRoleStateMachine.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/10/10
-- descrip:   战斗中其他玩家宠物角色状态机
--===================================================
local BattleOtherPetRoleStateMachine = class("BattleOtherPetRoleStateMachine",function()
    return require("StateMachine"):create()
end)

-- 构造函数
function BattleOtherPetRoleStateMachine:ctor()
    self._strName = "BattleOtherPetRoleStateMachine"         -- 状态机名称
    self._kTypeID = kType.kStateMachine.kBattleOtherPetRole  -- 状态类机型ID
...
-- 创建函数
function BattleOtherPetRoleStateMachine:create(master)
    local machine = BattleOtherPetRoleStateMachine.new()
    machine:onEnter(master)
    return machine
end

-- 进入函数
function BattleOtherPetRoleStateMachine:onEnter(master)
    --print(self._strName.." is onEnter!")
    self:setMaster(master)
    
    self:addState(require("BattleOtherPetRoleAppearState"):create())      -- 加入出场状态到状态机
    self:addState(require("BattleOtherPetRoleStandState"):create())       -- 加入站立状态到状态机
    self:addState(require("BattleOtherPetRoleRunState"):create())         -- 加入奔跑状态到状态机
    self:addState(require("BattleOtherPetRoleSkillAttackState"):create()) -- 加入技能攻击状态到状态机
    self:addState(require("BattleOtherPetRoleBebeatedState"):create())    -- 加入受击状态到状态机
    self:addState(require("BattleOtherPetRoleDeadState"):create())        -- 加入死亡状态到状态机
    self:addState(require("BattleOtherPetRoleDizzyState"):create())       -- 加入眩晕状态到状态机
    self:addState(require("BattleOtherPetRoleFrozenState"):create())      -- 加入冻结状态到状态机
    
    self:setCurStateByTypeID(kType.kState.kBattleOtherPetRole.kAppear)    -- 设置当前状态为出场状态
    
    return
end

-- 退出函数
function BattleOtherPetRoleStateMachine:onExit()
...
-- 更新逻辑
function BattleOtherPetRoleStateMachine:update(dt)
    if self._pCurState ~= nil then
        self._pCurState:update(dt)
    end

    if self:getMaster() then
        if self._pCurState._kTypeID ~= kType.kState.kBattleOtherPetRole.kDead and self:getMaster()._nCurHp > 0 then
...
return BattleOtherPetRoleStateMachine
