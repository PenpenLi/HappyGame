...
-- All Rights Reserved.
--==================================================
-- filename:  BattleOtherPetRoleRunState.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/10/10
-- descrip:   战斗中其他玩家宠物角色奔跑状态
--===================================================
local BattleOtherPetRoleRunState = class("BattleOtherPetRoleRunState",function()
    return require("State"):create()
end)

-- 构造函数
function BattleOtherPetRoleRunState:ctor()
    self._strName = "BattleOtherPetRoleRunState"           -- 状态名称
    self._kTypeID = kType.kState.kBattleOtherPetRole.kRun  -- 状态类型ID
...
-- 创建函数
function BattleOtherPetRoleRunState:create()
    local state = BattleOtherPetRoleRunState.new()
    return state
end

-- 进入函数
function BattleOtherPetRoleRunState:onEnter(args)
    --cclog("宠物奔跑")
...
-- 退出函数
function BattleOtherPetRoleRunState:onExit()
...
-- 更新逻辑
function BattleOtherPetRoleRunState:update(dt)
    if self._fIgnoreHurtTimeCount ~= - 1 then
...
-- 自动战斗相关逻辑
function BattleOtherPetRoleRunState:procAutoBattle(dt)
                ...
                if vSkill._pSkillInfo.PetSkillType == 4 then -- 回复性技能类型
                    if vSkill:isCDOver() == true then   -- 且已经CD结束，则使用该回复技能
                        ...
                        if #targetInWarning ~= 0 then  -- 警戒范围内有目标，则直接开始战斗
                            self:getMaster():getStateMachineByTypeID(kType.kStateMachine.kBattleOtherPetRole):setCurStateByTypeID(kType.kState.kBattleOtherPetRole.kSkillAttack, true, {["skillIndex"] = kSkill})
                            return 
                        end
                    end
                end
...
-- 奔跑逻辑
function BattleOtherPetRoleRunState:procRun(dt)      
...
return BattleOtherPetRoleRunState
