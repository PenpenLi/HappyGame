...
function BattleSkillProcessState:onEnter(args)
    ...
    if self:getMaster() then
        if self:getMaster():getMaster()._kRoleType == kType.kRole.kPlayer then
            -- mmo.DebugHelper:showJavaLog("--STATE--PLAYER_SKILL--:Process "..self:getMaster()._strName)
        elseif self:getMaster():getMaster()._kRoleType == kType.kRole.kMonster then
            -- mmo.DebugHelper:showJavaLog("--STATE--MONSTER_SKILL--:Process "..self:getMaster()._strName)
        elseif self:getMaster():getMaster()._kRoleType == kType.kRole.kPet then
            -- mmo.DebugHelper:showJavaLog("--STATE--PET_SKILL--:Process "..self:getMaster()._strName)
        end
