...
-- All Rights Reserved.
--==================================================
-- filename:  WorldOtherPlayerRoleRunState.lua
-- author:    taoye
-- e-mail:    553681974@qq.com
-- created:   2015/8/25
-- descrip:   世界中其他玩家角色奔跑状态
--===================================================
local WorldOtherPlayerRoleRunState = class("WorldOtherPlayerRoleRunState",function()
    return require("State"):create()
end)

-- 构造函数
function WorldOtherPlayerRoleRunState:ctor()
    self._strName = "WorldOtherPlayerRoleRunState"           -- 状态名称
    self._kTypeID = kType.kState.kWorldOtherPlayerRole.kRun  -- 状态类型ID
...
-- 创建函数
function WorldOtherPlayerRoleRunState:create()
    local state = WorldOtherPlayerRoleRunState.new()
    return state
end

-- 进入函数
function WorldOtherPlayerRoleRunState:onEnter(args)
    ...
    if self:getMaster() then
        ...
        if args ~= nil then
            ...
            elseif args.func ~= nil then -- 虽移动方向集合为0，但是回调函数不为空，则直接执行，返回即可
                args.func()
                self._pOwnerMachine:setCurStateByTypeID(kType.kState.kWorldOtherPlayerRole.kStand)
                return
            end
        end
...
-- 退出函数
function WorldOtherPlayerRoleRunState:onExit()
...
-- 更新逻辑
function WorldOtherPlayerRoleRunState:update(dt)
    if self:getMaster() then
        -- 奔跑逻辑
        self:procRun(dt)
        -- 检测遮挡
...
-- 奔跑逻辑
function WorldOtherPlayerRoleRunState:procRun(dt)
...
-- 检测遮挡
function WorldOtherPlayerRoleRunState:checkCover()
    self:getMaster():checkCover()
end

return WorldOtherPlayerRoleRunState