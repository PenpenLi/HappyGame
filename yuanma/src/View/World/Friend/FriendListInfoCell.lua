...
-- All Rights Reserved.
--==================================================
-- filename:  FriendListInfoCell.lua
-- author:    liyuhang
-- created:   2015/5/18
-- descrip:   好友列表cell
--===================================================
local FriendListInfoCell = class("FriendListInfoCell",function()
    return ccui.ImageView:create()
end)

-- 构造函数
function FriendListInfoCell:ctor()
    -- 层名称
    self._strName = "FriendListInfoCell"        
...
-- 创建函数
function FriendListInfoCell:create(dataInfo)
    local dialog = FriendListInfoCell.new()
    dialog:dispose(dataInfo)
    return dialog
end

-- 处理函数
function FriendListInfoCell:dispose(dataInfo)
    --注册（好友技能配置信息更新）
...
function FriendListInfoCell:initUI()
    --图标按钮
...
function FriendListInfoCell:updateData()
...
-- 整体到放大尺寸
function FriendListInfoCell:toBigScale()
    self:setScale(self._fBigScale)
end

-- 整体到正常尺寸
function FriendListInfoCell:toNormalScale()
    self:setScale(self._fNormalScale)
end

function FriendListInfoCell:setInfo(info)
...
-- 退出函数
function FriendListInfoCell:onExitFriendListInfoCell()
    NetRespManager:getInstance():removeEventListenersByHost(self)
    ResPlistManager:getInstance():removeSpriteFrames("FriendListInfo.plist")
end

function FriendListInfoCell:hanleMsgUpdateFriendSkill(event)
    if FriendManager:getInstance()._nMountFriendSkill ~= nil then
        if FriendManager:getInstance()._nMountFriendSkill.roleId == self._pDataInfo.roleId then
			self._pParams._pAlreadyIcon:setVisible(true)
		else
