...
function RoleCreateLayer:initRoles()
    ...
    for i=1, kCareer.kTotoalNum do
        ...
        local function appearOver()
            local actStand = cc.RepeatForever:create(cc.Animate3D:createWithFrames(animation, TableTempleteCareers[i].ReadyFightActFrameRegion[1], TableTempleteCareers[i].ReadyFightActFrameRegion[2]))
            pRole:runAction(actStand)
        end
        local appear = cc.Animate3D:createWithFrames(animation, TableTempleteCareers[i].LoginAppearActFrameRegion[1], TableTempleteCareers[i].LoginAppearActFrameRegion[2])
